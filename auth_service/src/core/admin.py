...
from src.core.models import User, Role, UserRoles, SocialAccount
...
class SocialAccountInfo(Model):
    key = CharField(max_length=64)
    value = CharField(max_length=64)

    social_account = ForeignKeyField(SocialAccount)
...
class SocialAccountAdmin(ModelView):  # type: ignore
    form_base_class = SecureForm
    inline_models = (SocialAccountInfo,)
